...
window.addEventListener('unhandledrejection', e => {
  const msg = e?.reason?.message || 'Erreur inattendue lors de la communication avec le serveur.';
  try { showStatus('Erreur serveur : ' + msg + ' — vérifiez votre connexion puis réessayez.', 'error'); } catch {}
});
window.addEventListener('error', e => {
  try { showStatus('Erreur de l’application : ' + (e.message || 'erreur inconnue'), 'error'); } catch {}
});
...
async function submitSurvey(){
  ...
  try{
    ...
    d.responses=d.responses||[];d.responses.push({...answers});/* demo disabled */(d);
...
async function checkApiV22(){
  try{
    const r=await api('/api/health');
    showStatus('Connexion serveur OK · D1 opérationnelle', 'success');
    return r;
  }catch(e){
    showStatus('Connexion serveur indisponible : '+e.message, 'error');
    throw e;
  }
}
